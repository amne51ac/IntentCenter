...
def _build_copilot_system_text(page_context: dict[str, Any] | None) -> str:
    ...
    return (
        ...
        "- **Filtered row lists (composable):** use **`catalog_list`** with a `query` id from the tool enum (e.g. "
        "`Device/without_interfaces` for devices with no non-deleted `Interface` rows). Optional `limit`. Returns `rows` "
        "for Markdown tables; `totalCount` / `truncated` apply. Same **composable** idea as `catalog_breakdown`, but for "
